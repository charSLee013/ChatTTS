...
class GFSQ(nn.Module):
    ...
    def forward(self, x,):
        ...
        return (
            torch.zeros(perplexity.shape, dtype=x.dtype, device=x.device),  # 复杂度
            feat.transpose(1,2) if self.transpose else feat,                # 量化后的特征
            perplexity, # 复杂度值
            None,       # 占位
            ind.transpose(1,2) if self.transpose else ind,  # 量化索引的元组
        )
